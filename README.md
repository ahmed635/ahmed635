<div align="center">

# 👋 Hi, I'm Ahmed Khaled

### 💻 Java & Flutter Full-Stack Developer | 🧠 Biomedical Engineer

🔭 Currently working at **Namasoft** | 📱 Published apps on Play Store & App Store

[![Facebook](https://img.shields.io/badge/Facebook-1877F2?style=for-the-badge&logo=facebook&logoColor=white)](https://www.facebook.com/ahmed.khaled.ali.970915/)
[![Gmail](https://img.shields.io/badge/Gmail-D14836?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ahmed.khaled.aa123@gmail.com)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ahmed-khaled-7bb818182/)

</div>

---

### 🔭 Working On
- Building mobile apps with **Flutter & Spring Boot**  
- Contributing to open-source projects

### 🌱 Currently Learning
- Advanced Spring Security  
- Microservices architecture

---

### 💼 Experience

**NamaSoft — Java Backend Engineer & Flutter Developer**  
📍 Cairo, Egypt · 🗓️ March 2024 – Present

Building the mobile-facing platform of a large-scale enterprise ERP (**Java 21, Spring Boot, Hibernate, MS SQL Server**) deployed at client organizations across Egypt and Saudi Arabia — owning features end to end, from domain model and API contract through to the Flutter clients that consume them.

- 🔌 **Backend APIs** — RESTful layer serving the customer and employee mobile apps: authentication, user management, sales and delivery documents, maintenance requests, customer visits, and HR self-service workflows.
- 🔐 **Auth & Security** — redesigned mobile authentication from session-based to **API Key**, with custom Spring Security filters, JWT, encrypted credential storage, role-based authorization, and biometric login.
- ⚡ **Real-Time & Notifications** — built a messaging platform over **WebSocket/STOMP** with authenticated handshake interception, plus a push service on **Firebase Cloud Messaging HTTP v1** using the OAuth2 service-account flow and a queued retry processor.
- 💳 **Payments & KitchenNet** — integrated **HyperPay** and related gateways; led *KitchenNet*, a white-label product line, as sole engineer on the standalone Spring Boot service and its Flutter client — shipped to **Google Play** and the **App Store**.
- 🧩 **ERP Modules & Performance** — HR, CRM, Point of Sale, Accounting, Supply Chain, Contracting, and Customer Service: geofenced electronic attendance, Bluetooth thermal receipt printing, application-level caching, and SQL/Hibernate tuning on high-traffic endpoints.
- 📱 **Cross-Platform Delivery** — own the Android and iOS release pipelines; delivered **three client brands** from a single Flutter codebase and led upgrades through Flutter 3.27, Android 15, and Play 16 KB page-size compliance.

---

### 🎓 Education & Certifications

- **B.Sc. Biomedical Engineering** — Helwan University, Cairo · 2018 – 2022  
  Graduation Project: *Smart Hospital System* (98%) · Overall: Very Good (82.5%)
- **Full Stack Spring Boot Diploma** — EraaSoft (Feb 2024 – Nov 2024)
- **Spring Boot 3, Spring Framework 6 & Hibernate** — Udemy
- **Java Programming Masterclass** — Udemy

---

### 💻 Tech Stack

**Backend:**
  
[![Java](https://img.shields.io/badge/Java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white)](https://www.java.com/en/)
[![Spring](https://img.shields.io/badge/Spring-%236DB33F.svg?style=for-the-badge&logo=spring&logoColor=white)](https://spring.io/)
[![Spring Boot](https://img.shields.io/badge/Spring_Boot-%236DB33F.svg?style=for-the-badge&logo=spring-boot&logoColor=white)](https://spring.io/projects/spring-security)
[![Spring Security](https://img.shields.io/badge/Spring_Security-%236DB33F.svg?style=for-the-badge&logo=springsecurity&logoColor=white)](https://spring.io/projects/spring-security)
[![Hibernate](https://img.shields.io/badge/Hibernate-%2359666C.svg?style=for-the-badge&logo=hibernate&logoColor=white)](https://hibernate.org/)
[![Apache Tomcat](https://img.shields.io/badge/Apache_Tomcat-%23F8DC75.svg?style=for-the-badge&logo=apachetomcat&logoColor=black)](https://tomcat.apache.org/)

**Databases:**

[![MS SQL](https://img.shields.io/badge/Microsoft%20SQL%20Server-%23CC2927.svg?style=for-the-badge&logo=microsoft%20sql%20server&logoColor=white)](https://www.microsoft.com/en-us/sql-server/sql-server-downloads?ocid=ORSEARCH_Bing&msockid=0af3369544a0632d0d92232c45dc62dd)
[![MySQL](https://img.shields.io/badge/MySQL-%234479A1.svg?style=for-the-badge&logo=mysql&logoColor=white)](https://www.mysql.com/)
[![SQLite](https://img.shields.io/badge/SQLite-%23003B57.svg?style=for-the-badge&logo=sqlite&logoColor=white)](https://www.sqlite.org/)

**Mobile:**

[![Flutter](https://img.shields.io/badge/Flutter-%2302569B.svg?style=for-the-badge&logo=Flutter&logoColor=white)](https://docs.flutter.dev/install/archive)
[![Dart](https://img.shields.io/badge/Dart-%230175C2.svg?style=for-the-badge&logo=dart&logoColor=white)](https://dart.dev/)
[![Firebase](https://img.shields.io/badge/Firebase-%23039BE5.svg?style=for-the-badge&logo=firebase)](https://firebase.google.com/)
[![Android](https://img.shields.io/badge/Android-3DDC84?style=for-the-badge&logo=android&logoColor=white)](https://www.android.com/)
[![iOS](https://img.shields.io/badge/iOS-000000?style=for-the-badge&logo=ios&logoColor=white)](https://www.apple.com/)
[![App Store](https://img.shields.io/badge/App_Store-0D96F6?style=for-the-badge&logo=app-store&logoColor=white)](https://www.apple.com/app-store/)
[![Google Play](https://img.shields.io/badge/Google_Play-414141?style=for-the-badge&logo=google-play&logoColor=white)](https://developer.android.com/distribute/console)

**Frontend:**

[![HTML5](https://img.shields.io/badge/HTML5-%23E34F26.svg?style=for-the-badge&logo=html5&logoColor=white)](https://www.w3schools.com/html/)
[![CSS3](https://img.shields.io/badge/CSS3-%231572B6.svg?style=for-the-badge&logo=css3&logoColor=white)](https://www.w3schools.com/css/default.asp)
[![JavaScript](https://img.shields.io/badge/JavaScript-%23323330.svg?style=for-the-badge&logo=javascript&logoColor=%23F7DF1E)](https://developer.mozilla.org/en-US/docs/Web/JavaScript)
[![TypeScript](https://img.shields.io/badge/TypeScript-%23007ACC.svg?style=for-the-badge&logo=typescript&logoColor=white)](https://www.typescriptlang.org/)
[![Vue.js](https://img.shields.io/badge/Vue.js-%2335495e.svg?style=for-the-badge&logo=vuedotjs&logoColor=%234FC08D)](https://vuejs.org/)
[![Angular](https://img.shields.io/badge/Angular-%23DD0031.svg?style=for-the-badge&logo=angular&logoColor=white)](https://angular.dev/)

**Tools:**

[![Git](https://img.shields.io/badge/Git-%23F05033.svg?style=for-the-badge&logo=git&logoColor=white)](https://git-scm.com/)
[![GitHub](https://img.shields.io/badge/GitHub-%23121011.svg?style=for-the-badge&logo=github&logoColor=white)](https://github.com/ahmed635)
[![Maven](https://img.shields.io/badge/Apache_Maven-%23C71A36.svg?style=for-the-badge&logo=apachemaven&logoColor=white)](https://maven.apache.org/)
[![Gradle](https://img.shields.io/badge/Gradle-%2302303A.svg?style=for-the-badge&logo=gradle&logoColor=white)](https://gradle.org/)
[![Postman](https://img.shields.io/badge/Postman-%23FF6C37.svg?style=for-the-badge&logo=postman&logoColor=white)](https://www.postman.com/)
[![IntelliJ IDEA](https://img.shields.io/badge/IntelliJ_IDEA-%23000000.svg?style=for-the-badge&logo=intellijidea&logoColor=white)](https://www.jetbrains.com/idea/)
